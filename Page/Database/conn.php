...
class User extends Conn
{
    ...
    function UserExists($svnr) {
        $query = "select user_id from user where svnr = ?";
    ...
    function GetMenuForWeek($week_offset = 0)
    {
        try {
            $query = "select menu_id, mv.food_id, mv.name, mv.meal_id, mv.type, date_format(mv.day, '%d.%m.%Y') `day`, mv.day_name, count(user_id) amount
                        ...
                        where day between date_add(current_date(), interval " . $week_offset . " day) and date_add(current_date(), interval " . ($week_offset + 7) . " day)
                        group by menu_id
                        order by  mv.day, meal_id;";
